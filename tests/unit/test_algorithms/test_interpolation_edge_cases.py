...
from materforge.parsing.config.yaml_keys import LINEAR_KEY, CONSTANT_KEY
...
class TestInterpolationComprehensive:
    ...
    def test_interpolate_value_equal_boundary_points(self):
        ...
        with pytest.raises(ValueError, match="Cannot extrapolate.*equal"):
            interpolate_value(250, x_array, y_array, LINEAR_KEY, CONSTANT_KEY)
    ...
    def test_interpolate_value_equal_last_points(self):
        ...
        with pytest.raises(ValueError, match="Cannot extrapolate.*equal"):
            interpolate_value(450, x_array, y_array, CONSTANT_KEY, LINEAR_KEY)
    ...
    def test_interpolate_value_at_exact_boundary(self):
        ...
        result = interpolate_value(300, x_array, y_array, CONSTANT_KEY, CONSTANT_KEY)
        assert result == 100.0
        # Exactly at upper boundary
        result = interpolate_value(500, x_array, y_array, CONSTANT_KEY, CONSTANT_KEY)
        assert result == 200.0
    ...
    def test_interpolate_value_very_close_to_boundary(self):
        ...
        result = interpolate_value(299.999, x_array, y_array, LINEAR_KEY, CONSTANT_KEY)
        expected = 100 + (150 - 100) * (299.999 - 300) / (400 - 300)
        assert abs(result - expected) < 1e-10
        # Very close to upper boundary (should extrapolate)
        result = interpolate_value(500.001, x_array, y_array, CONSTANT_KEY, LINEAR_KEY)
    ...
    def test_interpolate_value_extreme_values(self):
        ...
        result = interpolate_value(1e-6, x_array, y_array, CONSTANT_KEY, CONSTANT_KEY)
    ...
    def test_interpolate_value_negative_values(self):
        ...
        # Test within range
        result = interpolate_value(-400, x_array, y_array, CONSTANT_KEY, CONSTANT_KEY)
        expected = np.interp(-400, x_array, y_array)
        assert abs(result - expected) < 1e-10
        # Test extrapolation
        result = interpolate_value(-600, x_array, y_array, LINEAR_KEY, CONSTANT_KEY)
    ...
    def test_interpolate_value_above_range_constant(self):
        ...
        x_array = np.array([300, 400, 500])
        y_array = np.array([100, 150, 200])
        result = interpolate_value(550, x_array, y_array, CONSTANT_KEY, CONSTANT_KEY)
    ...
    def test_interpolate_value_below_range_extrapolate(self):
        ...
        x_array = np.array([300, 400, 500])
        y_array = np.array([100, 150, 200])
        result = interpolate_value(250, x_array, y_array, LINEAR_KEY, CONSTANT_KEY)
    ...
    def test_interpolate_value_above_range_extrapolate(self):
        ...
        x_array = np.array([300, 400, 500])
        y_array = np.array([100, 150, 200])
        result = interpolate_value(550, x_array, y_array, CONSTANT_KEY, LINEAR_KEY)
    ...
    def test_interpolate_value_nan_input(self):
        ...
        with pytest.raises(ValueError, match="dep_value must be finite, got nan"):
            interpolate_value(np.nan, x_array, y_array, CONSTANT_KEY, CONSTANT_KEY)
    ...
    def test_interpolate_value_inf_input(self):
        ...
        with pytest.raises(ValueError, match="dep_value must be finite"):
            interpolate_value(np.inf, x_array, y_array, CONSTANT_KEY, LINEAR_KEY)
    ...
    def test_interpolate_value_mismatched_array_lengths(self):
        ...
        with pytest.raises(ValueError):
            interpolate_value(350, x_array, y_array, CONSTANT_KEY, CONSTANT_KEY)
    ...
    def test_interpolate_value_single_point_array(self):
        ...
        result = interpolate_value(350, x_array, y_array, CONSTANT_KEY, CONSTANT_KEY)
        assert result == 100.0
        result = interpolate_value(250, x_array, y_array, LINEAR_KEY, LINEAR_KEY)
        assert result == 100.0
    ...
    def test_interpolate_value_boundary_conditions_comprehensive(self):
        ...
        boundary_combinations = [
            ...
            (CONSTANT_KEY, LINEAR_KEY),
            (LINEAR_KEY, CONSTANT_KEY),
