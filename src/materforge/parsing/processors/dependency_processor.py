...
from materforge.parsing.config.yaml_keys import INDEPENDENT_VARIABLES_KEY, DEPENDENCIES_KEY, EQUATION_KEY, RANGES_KEY
...
class DependencyProcessor:
    ...
    def process_computed_property(self, material: Material, prop_name: str,
        ...
        try:
            ...
            # Get primary dependency (computed properties have single dependency)
            dependencies = prop_config.get(DEPENDENCIES_KEY, [])
            if not dependencies:
                raise ValueError(f"No dependencies specified for computed property {prop_name}")
            ...
            try:
                material_property = self._parse_and_process_expression(
                    expression, material, dependency_symbols, prop_name
                )
            ...
            # Handle numeric dependency case using utility function
            if handle_numeric_dependency(self, material, prop_name, material_property, dependency_symbols):
                return
            # Evaluate the expression for symbolic dependencies
            primary_symbol = dependency_symbols[primary_dependency]
            # Get symbol mapping from material (if available)
            symbol_mapping = getattr(material, '_symbol_mapping', {primary_dependency: 'T'})
            yaml_symbol_name = symbol_mapping.get(primary_dependency, 'T')
            T_yaml = sp.Symbol(yaml_symbol_name)
            if isinstance(primary_symbol, sp.Symbol):
                # Replace YAML symbol with actual symbol
                standard_expr = material_property.subs(T_yaml, primary_symbol)
                f_pw = sp.lambdify(primary_symbol, standard_expr, 'numpy')
            else:
                f_pw = sp.lambdify(T_yaml, material_property, 'numpy')
            try:
                ...
                if self.property_handler is not None:
                    ...
                    self.property_handler.finalize_computed_property(
                        material, prop_name, temp_array, y_dense, dependency_symbols, prop_config
                    )
                else:
    ...
    def _parse_and_process_expression(self, expression: str, material: Material,
                                      dependency_symbols: Dict[str, sp.Symbol], prop_name: str) -> sp.Expr:
        ...
        try:
            ...
            if hasattr(material, '_parser_config') and INDEPENDENT_VARIABLES_KEY in material._parser_config:
                symbol_mapping = material._parser_config[INDEPENDENT_VARIABLES_KEY]
            else:
                # Fallback: assume temperature mapping
                symbol_mapping = {'temperature': 'T'}
            # Create mapping of YAML symbols to actual symbols
            yaml_to_actual = {}
            for dep_name, actual_symbol in dependency_symbols.items():
                if dep_name in symbol_mapping:
                    ...
                    yaml_to_actual[sp.Symbol(yaml_symbol_name)] = actual_symbol
            sympy_expr = sp.sympify(expression, evaluate=False)
            # Extract dependencies (exclude YAML symbols)
            yaml_symbols = set(symbol_mapping.values())
            dependencies = [str(symbol) for symbol in sympy_expr.free_symbols
                            if str(symbol) not in yaml_symbols]
            if dependencies:
                ...
                for dep in dependencies:
                    if not hasattr(material, dep) or getattr(material, dep) is None:
                        if dep in self.properties:
                            ...
                            self.process_computed_property(material, dep, dependency_symbols)
                        else:
                            available_props = sorted(list(self.properties.keys()))
                ...
                # Apply dependency substitutions
                result_expr = sympy_expr.subs(substitutions)
            else:
                result_expr = sympy_expr
            # Apply YAML symbol to actual symbol substitutions
            result_expr = result_expr.subs(yaml_to_actual)
            # Handle integrals
    ...
    def _validate_circular_dependencies(self, prop_name: str, current_deps: List[str],
        ...
        if path is None:
            path = []
        # Filter out YAML symbols from dependencies
        yaml_symbols = {'T', 'P', 'S', 'C', 't'}
        current_deps = [dep for dep in current_deps if dep not in yaml_symbols]
    ...
    def _extract_equation_dependencies(equation_data) -> List[str]:
        ...
        if isinstance(equation_data, list):
            ...
            symbols.update(expr.free_symbols)
        # Filter out YAML symbols, keep only property dependencies
        return [str(symbol) for symbol in symbols if str(symbol) not in yaml_symbols]
